...
Описание:
Этот файл принимает на вход массив страйков, массив DA и параметры отбора, а затем считает
стоимость контракта с объёмной гибкостью.
Входные данные:
max_acq: Максимальный годовой отбор
min_acq: Минимальный годовой отбор
max_dcq: Максимальный дневной отбор
min_dcq: Минимальный дневной отбор
delivery_days: количество дней от начала до конца поставки
underlying_paths: симуляции DA
strike_paths: симуляции индексов
premium: премия
fine_function=infinite_fine: функция штрафа
Выходные данные:
Массив из стоимости контракта и матрицы принятия решений контрагентом
"""
...
def lsmc(max_acq, min_acq, max_dcq, min_dcq, delivery_days, underlying_paths, strike_paths,
    ...
    # TODO ! REFACTORING NEEDED. TO NP.REPEAT NOT LOOP
    ...
    for z in range(Z - 2, -1, -1):
        ...
        expected_receive_values = payoffs[:, :, z] * max_dcq + shift(array=npv_forecast[:, :, z + 1], lag=-1,
                                                                     fill_value=-np.inf) # max withdrawal
        expected_reject_values = payoffs[:, :, z] * min_dcq + npv_forecast[:, :, z + 1] # we need this to eliminate min withdrawal
        ...
        # TODO !Подумать, как ускорить, чтобы продолжало работать с Numba
        for x in range(X):
            for y in range(Y):
